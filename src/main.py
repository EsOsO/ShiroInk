...
if __name__ == "__main__":
    ...
    if args.list_devices:
        ...
        # Print header
        print(f"{'Device':<32} {'Resolution':<12} {'Size':<7} {'Display':<8} {'Color':<8} {'Gamut':<10} {'Bits':<5} {'Pipeline':<20}")
        print("-" * 140)
        ...
        for brand, device_list in brands.items():
            if device_list:
                for key, spec in device_list:
                    ...
                    print(f"{key:<32} {spec.resolution[0]:4d}x{spec.resolution[1]:<7d} "
    ...
    if args.device and args.resolution is not None:
        ...
        print("The --device preset automatically sets the optimal resolution for that device")
    ...
    if args.device:
        ...
        try:
            ...
            print(f"Color support:    {'Color' if device_spec.color_support else 'B&W only'}")
            if device_spec.color_gamut:
                print(f"Color gamut:      {device_spec.color_gamut.value}")
            print(f"Bit depth:        {device_spec.bit_depth}-bit ({device_spec.max_colors if device_spec.max_colors else 'grayscale'} colors)")
